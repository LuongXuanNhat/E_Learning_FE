...
import { createSchedule, getScheduleById } from "@/services/service";
import {
  Card,
  Input,
  Button,
...
function IndexPage({ params }: { params: { id: number } }) {
  ...
  const [faculty, setSchedule] = useState<Schedule>({
    schedule_id: 0,
    description: "",
    name: "",
    created_at: new Date(),
  ...
  const validateData = () => {
    const requiredFields: (keyof Schedule)[] = ["name", "description"];
    ...
    if (faculty.name === "") {
      addAlert(AlertType.warning, "Hãy chọn lịch");
      return true;
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await createSchedule(faculty);
      addAlert(AlertType.success, `Thêm mới lịch ${faculty.name} thành công`);
      faculty.description = "";
      faculty.name = "";
    } catch (error) {
      ...
      addAlert(
        AlertType.error,
        `Thêm mới lịch ${faculty.name} thất bại: ` + error
      );
    }
  };
  const loadSchedule = async () => {
    try {
      ...
      addAlert(AlertType.error, "Có lỗi xảy ra khi tải dữ liệu lịch.");
      setError("");
      setLoading(false);
    }
  };
