...
import {
  deleteUser,
  fetchTeacherDepartmentPersonal,
  fetchTrainingDepartmentPersonal,
  fetchUsers,
...
function EmployeeSecretary() {
  ...
  useEffect(() => {
    ...
    const loadUsers = async () => {
      try {
        const data = await fetchTeacherDepartmentPersonal();
        if (userCurrent?.role === Position.SECRETARY) {
          const teachers = data.filter(
            (user) => user.faculty_id === userCurrent.faculty_id
          );
          setUsers(teachers);
        } else {
          setUsers(data);
...
export default MiddlewareAuthor(EmployeeSecretary, [
