...
export default function RegistedCourse({
  ...
  async function CancelRegisCourse(classItem: Class) {
    try {
      ...
      const removeClass = classes.filter((item) => {
        return item.class_id != classItem.class_id;
      });
      setClases(removeClass);
      addAlert(
        AlertType.success,
            ...
            <tr key={classes.class_id}>
              ...
              <td className="p-4 border-b border-blue-gray-50 flex justify-center">
                <Button
                  ...
                  onClick={() => CancelRegisCourse(classes)}
                  disabled={(() => {
                    if (!classes.Course?.registration_deadline) return true;
                    const deadline = new Date(
                      classes.Course.registration_deadline
                    );
                    return (
